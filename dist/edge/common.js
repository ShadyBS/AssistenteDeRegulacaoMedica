...
class KeepAliveManager {
  ...
  loadSettings() {
    ...
    return (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
      try {
        ...
        const result = yield api.storage.sync.get({
          keepSessionAliveInterval: 10
        });
        _this2.updateInterval(result.keepSessionAliveInterval);
      } catch (error) {
  ...
  start() {
    ...
    this.intervalId = setInterval(/*#__PURE__*/(0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
      try {
        const success = yield _api_js__WEBPACK_IMPORTED_MODULE_1__.keepSessionAlive();
        if (success) {
...
/* harmony export */ });
/* harmony import */ var bluebird__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! bluebird */ "./node_modules/bluebird/js/browser/bluebird.js");
/* harmony import */ var _filter_config_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./filter-config.js */ "./filter-config.js");
...
class SectionManager {
  ...
  init() {
    ...
    _store_js__WEBPACK_IMPORTED_MODULE_4__.store.subscribe(() => this.onStateChange());
  }
  onStateChange() {
    var _this$currentPatient, _this$currentPatient$, _newPatient$isenPK;
    const patientState = _store_js__WEBPACK_IMPORTED_MODULE_4__.store.getPatient();
  ...
  addEventListeners() {
    ...
    (_this$elements$sectio = this.elements.section) === null || _this$elements$sectio === void 0 ? void 0 : _this$elements$sectio.addEventListener('input', _utils_js__WEBPACK_IMPORTED_MODULE_2__.debounce(e => {
  ...
  fetchData() {
    ...
    return (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
      if (!_this.currentPatient) {
        if (_this.elements.section.style.display !== 'none') _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage('Nenhum paciente selecionado.');
        return;
      }
      if (_this.isLoading) return;
      _this.isLoading = true;
      _this.elements.content.innerHTML = '<p class="text-slate-500">Carregando...</p>';
      try {
        var _this$elements$mainFi;
        const fetchTypeElement = (_this$elements$mainFi = _this.elements.mainFilters) === null || _this$elements$mainFi === void 0 ? void 0 : _this$elements$mainFi.querySelector(`#${_this.prefix}-fetch-type-buttons`);
        if (fetchTypeElement) {
          _this.fetchType = fetchTypeElement.value;
        }
        const dataInicialValue = _this.elements.dateInitial ? _this.elements.dateInitial.value : null;
        const dataFinalValue = _this.elements.dateFinal ? _this.elements.dateFinal.value : null;
        const params = {
          ...
          type: _this.fetchType
        };
        if (_this.sectionKey === 'exams') {
          params.comResultado = _this.fetchType === 'withResult' || _this.fetchType === 'all';
          params.semResultado = _this.fetchType === 'withoutResult' || _this.fetchType === 'all';
        }
        ...
        _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage(`Erro ao buscar ${friendlyName}. Verifique a conexão e a URL base.`);
        _this.allData = [];
      } finally {
        _this.isLoading = false;
        _this.applyFiltersAndRender();
      }
    })();
  }
  ...
  sortData(data) {
    ...
    return [...data].sort((a, b) => {
      ...
      if (key === 'date' || key === 'sortableDate') {
        valA = a.sortableDate || _utils_js__WEBPACK_IMPORTED_MODULE_2__.parseDate(a.date);
        valB = b.sortableDate || _utils_js__WEBPACK_IMPORTED_MODULE_2__.parseDate(b.date);
      } else {
  ...
  getFilterValues() {
    const values = {};
    const filters = _filter_config_js__WEBPACK_IMPORTED_MODULE_1__.filterConfig[this.sectionKey] || [];
  ...
  clearFilters(shouldRender = true) {
    ...
    if (this.elements.dateInitial) this.elements.dateInitial.valueAsDate = _utils_js__WEBPACK_IMPORTED_MODULE_2__.calculateRelativeDate(range.start);
    if (this.elements.dateFinal) this.elements.dateFinal.valueAsDate = _utils_js__WEBPACK_IMPORTED_MODULE_2__.calculateRelativeDate(range.end);
    // --- FIM DA CORREÇÃO ---

    (_filter_config_js__WEBPACK_IMPORTED_MODULE_1__.filterConfig[this.sectionKey] || []).forEach(filter => {
      if (filter.type === 'component') return;
      const el = document.getElementById(filter.id);
      if (el) {
  ...
  saveFilterSet() {
    ...
    if (!name || name.trim() === '') {
      _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage('Nome inválido. O filtro não foi salvo.');
      return;
    }
    const savedSets = _store_js__WEBPACK_IMPORTED_MODULE_4__.store.getSavedFilterSets();
    ...
    _store_js__WEBPACK_IMPORTED_MODULE_4__.store.setSavedFilterSets(savedSets);
    _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage(`Filtro "${name}" salvo com sucesso.`, 'success');
  }
  loadFilterSet() {
    const select = document.getElementById(`${this.prefix}-saved-filters-select`);
    const name = select.value;
    if (!name) return;
    const set = (_store_js__WEBPACK_IMPORTED_MODULE_4__.store.getSavedFilterSets()[this.sectionKey] || []).find(s => s.name === name);
    if (!set) return;
  ...
  deleteFilterSet() {
    ...
    const name = select.value;
    if (!name) {
      _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage('Selecione um filtro para apagar.');
      return;
    }
    ...
    _store_js__WEBPACK_IMPORTED_MODULE_4__.store.setSavedFilterSets(savedSets);
    _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage(`Filtro "${name}" apagado.`, 'success');
  }
  populateSavedFilterDropdown() {
    ...
    const sets = _store_js__WEBPACK_IMPORTED_MODULE_4__.store.getSavedFilterSets()[this.sectionKey] || [];
    sets.forEach(set => {
  ...
  renderFilterControls() {
    try {
      const sectionFilters = _filter_config_js__WEBPACK_IMPORTED_MODULE_1__.filterConfig[this.sectionKey] || [];
      const sectionLayout = this.globalSettings.filterLayout[this.sectionKey] || [];
      const layoutMap = new Map(sectionLayout.map(f => [f.id, f]));
  ...
  applyAutomationFilters(filterSettings, ruleName) {
    ...
    if (filterSettings.dateRange) {
      ...
      if (this.elements.dateInitial && start !== null && !isNaN(start)) {
        this.elements.dateInitial.valueAsDate = _utils_js__WEBPACK_IMPORTED_MODULE_2__.calculateRelativeDate(start);
      }
      if (this.elements.dateFinal && end !== null && !isNaN(end)) {
        this.elements.dateFinal.valueAsDate = _utils_js__WEBPACK_IMPORTED_MODULE_2__.calculateRelativeDate(end);
      }
    }
    // --- FIM DA CORREÇÃO ---
...
class TimelineManager {
  ...
  addEventListeners() {
    ...
    (_this$elements$fetchB = this.elements.fetchBtn) === null || _this$elements$fetchB === void 0 ? void 0 : _this$elements$fetchB.addEventListener('click', () => this.fetchData());
    (_this$elements$toggle = this.elements.toggleBtn) === null || _this$elements$toggle === void 0 ? void 0 : _this$elements$toggle.addEventListener('click', () => this.toggleSection());
    (_this$elements$search = this.elements.searchKeyword) === null || _this$elements$search === void 0 ? void 0 : _this$elements$search.addEventListener('input', _utils_js__WEBPACK_IMPORTED_MODULE_2__.debounce(() => this.render(), 300));
  ...
  fetchData() {
    ...
    return (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
      ...
      _this.isLoading = true;
      _renderers_js__WEBPACK_IMPORTED_MODULE_3__.renderTimeline([], 'loading');
      try {
        const params = {
          isenPK: `${_this.currentPatient.isenPK.idp}-${_this.currentPatient.isenPK.ids}`,
          isenFullPKCrypto: _this.currentPatient.isenFullPKCrypto,
        ...
        const apiData = yield _api_js__WEBPACK_IMPORTED_MODULE_1__.fetchAllTimelineData(params);
        const normalizedData = _utils_js__WEBPACK_IMPORTED_MODULE_2__.normalizeTimelineData(apiData);
        _this.allData = normalizedData;
        _this.render();
      } catch (error) {
        console.error('Erro ao buscar dados para a Linha do Tempo:', error);
        _renderers_js__WEBPACK_IMPORTED_MODULE_3__.renderTimeline([], 'error');
      } finally {
        _this.isLoading = false;
      }
    })();
  ...
  getFilterValues() {
    ...
    return {
      ...
      keyword: _utils_js__WEBPACK_IMPORTED_MODULE_2__.normalizeString(((_this$elements$search2 = this.elements.searchKeyword) === null || _this$elements$search2 === void 0 ? void 0 : _this$elements$search2.value) || '')
    };
  }
  render() {
    if (this.allData.length === 0 && !this.isLoading) {
      _renderers_js__WEBPACK_IMPORTED_MODULE_3__.renderTimeline([], 'empty');
      return;
    }
    ...
    if (this.isFilteredView && this.activeRuleFilters) {
      dataToRender = _utils_js__WEBPACK_IMPORTED_MODULE_2__.filterTimelineEvents(dataToRender, this.activeRuleFilters);
    }
    _renderers_js__WEBPACK_IMPORTED_MODULE_3__.renderTimeline(dataToRender, 'success');
  }
  toggleSection() {
    var _this$elements$wrappe;
    (_this$elements$wrappe = this.elements.wrapper) === null || _this$elements$wrappe === void 0 ? void 0 : _this$elements$wrappe.classList.toggle('show');
...
function _getBaseUrl() {
  _getBaseUrl = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
    let data;
    try {
      data = yield api.storage.sync.get('baseUrl');
    } catch (e) {
 ...
 */
function fetchRegulationPriorities() {
  return _fetchRegulationPriorities.apply(this, arguments);
}
 ...
 */
function _fetchRegulationPriorities() {
  _fetchRegulationPriorities = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
...
function fetchRegulationDetails(_x) {
  return _fetchRegulationDetails.apply(this, arguments);
}
function _fetchRegulationDetails() {
  _fetchRegulationDetails = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    reguIdp,
    reguIds
  }) {
    if (!reguIdp || !reguIds) {
...
function searchPatients(_x2) {
  return _searchPatients.apply(this, arguments);
}
function _searchPatients() {
  _searchPatients = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* (term) {
...
function fetchVisualizaUsuario(_x3) {
  return _fetchVisualizaUsuario.apply(this, arguments);
}
function _fetchVisualizaUsuario() {
  _fetchVisualizaUsuario = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchProntuarioHash(_x4) {
  return _fetchProntuarioHash.apply(this, arguments);
}
function _fetchProntuarioHash() {
  _fetchProntuarioHash = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenFullPKCrypto,
    dataInicial,
    dataFinal
  }) {
...
function fetchConsultasEspecializadas(_x5) {
  return _fetchConsultasEspecializadas.apply(this, arguments);
}
function _fetchConsultasEspecializadas() {
  _fetchConsultasEspecializadas = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenFullPKCrypto,
    dataInicial,
    dataFinal
  }) {
...
function fetchConsultasBasicas(_x6) {
  return _fetchConsultasBasicas.apply(this, arguments);
}
function _fetchConsultasBasicas() {
  _fetchConsultasBasicas = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenFullPKCrypto,
    dataInicial,
    dataFinal
  }) {
...
function fetchAllConsultations(_x7) {
  return _fetchAllConsultations.apply(this, arguments);
}
function _fetchAllConsultations() {
  _fetchAllConsultations = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenFullPKCrypto,
    dataInicial,
    dataFinal
  }) {
    const [basicasResult, especializadasResult] = yield Promise.all([fetchConsultasBasicas({
      isenFullPKCrypto,
      dataInicial,
      dataFinal
...
function fetchExamesSolicitados(_x8) {
  return _fetchExamesSolicitados.apply(this, arguments);
}
function _fetchExamesSolicitados() {
  _fetchExamesSolicitados = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK,
    dataInicial,
    dataFinal,
    comResultado,
...
function fetchResultadoExame(_x9) {
  return _fetchResultadoExame.apply(this, arguments);
}
function _fetchResultadoExame() {
  _fetchResultadoExame = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchCadsusData(_x0) {
  return _fetchCadsusData.apply(this, arguments);
}
function _fetchCadsusData() {
  _fetchCadsusData = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    cpf,
    cns
  }) {
    if (!cpf && !cns) {
...
function fetchAppointmentDetails(_x1) {
  return _fetchAppointmentDetails.apply(this, arguments);
}
 ...
 */
function _fetchAppointmentDetails() {
  _fetchAppointmentDetails = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchExamAppointmentDetails(_x10) {
  return _fetchExamAppointmentDetails.apply(this, arguments);
}
function _fetchExamAppointmentDetails() {
  _fetchExamAppointmentDetails = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchAppointments(_x11) {
  return _fetchAppointments.apply(this, arguments);
}
function _fetchAppointments() {
  _fetchAppointments = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK,
    dataInicial,
    dataFinal
  }) {
    ...
    for (let i = 0; i < basicAppointments.length; i += batchSize) {
      ...
      const promises = batch.map(/*#__PURE__*/function () {
        var _ref = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* (appt) {
          ...
          try {
            const details = yield fetchAppointmentDetails({
              idp,
              ids
            });
            if (details) {
      ...
      const settledBatch = yield Promise.all(promises);
...
function fetchRegulations(_x12) {
  return _fetchRegulations.apply(this, arguments);
}
function _fetchRegulations() {
  _fetchRegulations = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK,
    modalidade,
    dataInicial,
    dataFinal
...
function fetchAllRegulations(_x13) {
  return _fetchAllRegulations.apply(this, arguments);
}
 ...
 */
function _fetchAllRegulations() {
  _fetchAllRegulations = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK,
    dataInicial,
    dataFinal,
    type = 'all'
    ...
    if (type === 'all') {
      regulationsToFetch = yield Promise.all([fetchRegulations({
        isenPK,
        modalidade: 'ENC',
        dataInicial,
        dataFinal
      ...
      regulationsToFetch = [yield fetchRegulations({
        isenPK,
        modalidade: 'ENC',
        dataInicial,
        dataFinal
    ...
    const regulationsWithAttachments = yield Promise.all(allRegulations.map(/*#__PURE__*/function () {
      var _ref2 = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* (regulation) {
        if (regulation.idp && regulation.ids) {
          try {
            ...
            const attachments = yield fetchRegulationAttachments({
...
function _fetchDocuments() {
  _fetchDocuments = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK
  }) {
    if (!isenPK) throw new Error('ID (isenPK) do paciente é necessário.');
...
function fetchDocumentUrl(_x15) {
  return _fetchDocumentUrl.apply(this, arguments);
}
 ...
 */
function _fetchDocumentUrl() {
  _fetchDocumentUrl = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchRegulationAttachments(_x16) {
  return _fetchRegulationAttachments.apply(this, arguments);
}
 ...
 * @returns {Promise<string|null>} A URL completa para visualização do arquivo.
 */
function _fetchRegulationAttachments() {
  _fetchRegulationAttachments = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    reguIdp,
    reguIds,
    isenPK
  }) {
...
function fetchRegulationAttachmentUrl(_x17) {
  return _fetchRegulationAttachmentUrl.apply(this, arguments);
}
 ...
 */
function _fetchRegulationAttachmentUrl() {
  _fetchRegulationAttachmentUrl = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    idp,
    ids
  }) {
...
function fetchAllTimelineData(_x18) {
  return _fetchAllTimelineData.apply(this, arguments);
}
 ...
 */
function _fetchAllTimelineData() {
  _fetchAllTimelineData = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* ({
    isenPK,
    isenFullPKCrypto,
    dataInicial,
    dataFinal
    ...
    const results = yield Promise.allSettled(Object.values(dataPromises));
...
function keepSessionAlive() {
  return _keepSessionAlive.apply(this, arguments);
}
function _keepSessionAlive() {
  _keepSessionAlive = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
    try {
      const baseUrl = yield getBaseUrl();
...
function renderRecentPatients() {
  ...
  const recents = _store_js__WEBPACK_IMPORTED_MODULE_3__.store.getRecentPatients() || [];
...
function renderPatientListItem(patient) {
  ...
  const nome = patient.value || _utils_js__WEBPACK_IMPORTED_MODULE_2__.getNestedValue(patient, 'entidadeFisica.entidade.entiNome') || 'Nome não informado';
  ...
  const dataNascimento = patient.dataNascimento || _utils_js__WEBPACK_IMPORTED_MODULE_2__.getNestedValue(patient, 'entidadeFisica.entfDtNasc');
  const cpf = patient.cpf || _utils_js__WEBPACK_IMPORTED_MODULE_2__.getNestedValue(patient, 'entidadeFisica.entfCPF');
...
const handleSearchInput = _utils_js__WEBPACK_IMPORTED_MODULE_2__.debounce(/*#__PURE__*/(0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* () {
  const searchTerm = searchInput.value.trim();
  _store_js__WEBPACK_IMPORTED_MODULE_3__.store.clearPatient();
  ...
  _utils_js__WEBPACK_IMPORTED_MODULE_2__.toggleLoader(true);
  try {
    const patients = yield _api_js__WEBPACK_IMPORTED_MODULE_1__.searchPatients(searchTerm);
    renderSearchResults(patients);
  } catch (error) {
    _utils_js__WEBPACK_IMPORTED_MODULE_2__.showMessage('Erro ao buscar pacientes.');
  } finally {
    _utils_js__WEBPACK_IMPORTED_MODULE_2__.toggleLoader(false);
  }
}), 500);
...
function handleResultClick(_x) {
  return _handleResultClick.apply(this, arguments);
}
function _handleResultClick() {
  _handleResultClick = (0,bluebird__WEBPACK_IMPORTED_MODULE_0__.coroutine)(function* (event) {
    ...
    if (listItem.classList.contains('recent-patient-item')) {
      const recentPatient = _store_js__WEBPACK_IMPORTED_MODULE_3__.store.getRecentPatients().find(p => {
      ...
      if (recentPatient && recentPatient.ficha) {
        _store_js__WEBPACK_IMPORTED_MODULE_3__.store.setPatient(recentPatient.ficha, recentPatient.cadsus);
        searchInput.value = '';
        searchResultsList.classList.add('hidden');
        recentPatientsList.classList.add('hidden');
        return;
...
function init(config) {
  ...
  _store_js__WEBPACK_IMPORTED_MODULE_3__.store.subscribe(() => {
