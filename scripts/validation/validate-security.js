...
class SecurityValidator {
  ...
  async validateManifestSecurity() {
    ...
    contentScripts.forEach((script, index) => {
      if (script.all_frames === true) {
        this.addSecurityIssue(`Content script ${index} runs in all frames - potential security risk`, 'MEDIUM');
      }
      
      if (script.run_at === 'document_start') {
        this.addSecurityIssue(`Content script ${index} runs at document_start - ensure input validation`, 'LOW');
      }
    });
  }
  ...
  async analyzeScriptSecurity(scriptPath) {
    ...
    const logPattern = /console\.(log|warn|error|info)\s*\([^)]*(?:cpf|password|token|key|secret|auth)[^)]*\)/gi;
  ...
  async validatePermissionSecurity() {
    ...
    if (allPermissions.length > 10) {
      this.addSecurityIssue(`Large number of permissions (${allPermissions.length}) - review if all are necessary`, 'MEDIUM');
    }
    ...
    if (hasPrivacyPermissions) {
      this.addSecurityIssue('Privacy-sensitive permissions detected - ensure GDPR/LGPD compliance', 'MEDIUM');
    }
  ...
  async validateDependencySecurity() {
    ...
    if (depCount > 50) {
      this.addSecurityIssue(`Large number of dependencies (${depCount}) increases attack surface`, 'LOW');
    }
  }
  ...
  printResults() {
    ...
    console.log(`🚨 High Severity Issues: ${this.securityIssues.filter(i => i.severity === 'HIGH').length}`);
    console.log(`⚠️ Medium Severity Issues: ${this.securityIssues.filter(i => i.severity === 'MEDIUM').length}`);
    console.log(`ℹ️ Low Severity Issues: ${this.securityIssues.filter(i => i.severity === 'LOW').length}`);
